...
for (season in unique(top_run_getters$Season)) {
  ...
  ashwin_data <- performance_salary_data %>% filter(Player == "R Ashwin")
  print("R Ashwin's Performance and Salary Data:")
  print(ashwin_data)
  ggplot(ashwin_data, aes(x = Salary)) +
    geom_bar(aes(y = total_runs), stat = "identity", fill = "blue", alpha = 0.7) +
    geom_bar(aes(y = total_wickets), stat = "identity", fill = "red", alpha = 0.7) +
    labs(title = "R Ashwin's Performance Metrics and Salary",
         x = "Salary",
